...
public class DepartmentHome {
	...
	public Department findById(java.lang.Long id) {
		...
		try {
			Department instance = (Department) sessionFactory
					.getCurrentSession().get("com.sur.balaji.model.Department",
							id);
			if (instance == null) {
	...
	public List<Department> findByExample(Department instance) {
		...
		try {
			List<Department> results = (List<Department>) sessionFactory
					.getCurrentSession()

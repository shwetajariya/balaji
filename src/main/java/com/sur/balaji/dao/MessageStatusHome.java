...
public class MessageStatusHome {
	...
	public MessageStatus merge(MessageStatus detachedInstance) {
		...
		try {
			MessageStatus result = (MessageStatus) sessionFactory
	...
	public MessageStatus findById(MessageStatusId id) {
		...
		try {
			MessageStatus instance = (MessageStatus) sessionFactory
					.getCurrentSession().get(
							"com.sur.balaji.dao.MessageStatus", id);
			if (instance == null) {
	...
	public List<MessageStatus> findByExample(MessageStatus instance) {
		...
		try {
			List<MessageStatus> results = (List<MessageStatus>) sessionFactory
					.getCurrentSession()
